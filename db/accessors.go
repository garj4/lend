...
func AddPerson(firstName, lastName string) error {
	...
	_, err = database.exec(addPersonQuery, firstName, lastName)
	if err != nil {
		return err
	}
...
func AddTransaction(event, firstName string, amount float64) error {
	...
	_, err = database.exec(addTransactionQuery, event, formattedAmount, formattedTime, formattedPersonID)
	if err != nil {
		return err
	}
...
func PrintPeople(w io.Writer) error {
	rows, err := database.query(selectAllPeopleQuery)
	if err != nil {
		return err
	}
	defer rows.Close()
...
func PrintTransactions(w io.Writer) error {
	rows, err := database.query(selectAllTransactionsQuery)
	if err != nil {
		return err
	}
	defer rows.Close()
...
func GetAmountOwed(name string) (float64, error) {
	...
	rows, err := database.query(sumTransactionsQuery, personID)
	if err != nil {
		return 0, err
	}
	defer rows.Close()
